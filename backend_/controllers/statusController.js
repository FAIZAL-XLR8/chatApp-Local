...
exports.createStatus = async (req, res) => {
  try {
    ...
    if (req.io && req.socketUserMap) {
      // Emit to all online users (simplified broadcast)
      for (const [connectedUserId, socketIds] of req.socketUserMap.entries()) {
        if (connectedUserId.toString() !== userId.toString()) {
          req.io.to(connectedUserId.toString()).emit("new-status", populatedStatus);
        }
      }
    }
...
exports.deleteStatus = async (req, res) => {
  try {
    ...
    if (req.io && req.socketUserMap) {
      for (const [connectedUserId, socketIds] of req.socketUserMap.entries()) {
        if (connectedUserId.toString() !== userId.toString()) {
          req.io.to(connectedUserId.toString()).emit("status-deleted", statusId);
        }
      }
    }
