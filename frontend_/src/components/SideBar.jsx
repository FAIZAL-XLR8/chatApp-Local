...
const SideBar = () => {
  ...
  const SidebarContent = (
    <>
      <Link
        to="/"
        className={`${isMobile ? "" : "mb-6"} ${activeTab === "chats" &&
          "bg-gray-100 shadow-sm border border-gray-300 p-3 rounded-full"
          } focus:outline-none`}
      >
        <FaComments
          className={`h-7 w-7 ${activeTab === "chats"
            ? "text-indigo-600"
            : "text-gray-500"
            }`}
        />
      ...
      <Link
        to="/status"
        className={`${isMobile ? "" : "mb-6"} ${activeTab === "status" &&
          "bg-gray-100 shadow-sm border border-gray-300 p-3 rounded-full"
          } focus:outline-none`}
      >
        <MdOutlineRadioButtonChecked
          className={`h-7 w-7 ${activeTab === "status"
            ? "text-gray-900"
            : "text-gray-500"
            }`}
        />
      ...
      <Link
        to="/user-profile"
        className={`${isMobile ? "" : "mb-6"} ${activeTab === "profile" &&
          "bg-gray-100 shadow-sm border border-gray-300 p-3 rounded-full"
          } focus:outline-none`}
      >
        {user?.user?.profilePicture ? (
          ...
          <FaUserCircle
            className={`h-7 w-7 ${activeTab === "profile"
              ? "text-gray-900"
              : "text-gray-500"
              }`}
          />
      ...
      <Link
        to="/setting"
        className={`${isMobile ? "" : "mb-6"} ${activeTab === "setting" &&
          "bg-gray-100 shadow-sm border border-gray-300 p-3 rounded-full"
          } focus:outline-none`}
      >
        <FaCog
          className={`h-7 w-7 ${activeTab === "setting"
            ? "text-gray-900"
            : "text-gray-500"
            }`}
        />
