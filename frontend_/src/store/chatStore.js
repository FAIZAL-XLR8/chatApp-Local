...
export const useChatStore = create((set, get) => ({
  ...
  initialSocketListeners: () => {
    ...
    socket.on("message-status-update", ({ messageId, messageStatus }) => {
      console.log("📨 Message status update received:", { messageId, messageStatus });
      set((state) => ({
    ...
    socket.on("messages-read", ({ messageIds, messageStatus }) => {
      set((state) => ({
        messages: state.messages.map((msg) =>
  ...
  receiveMessage: (message) => {
    ...
    const userStoreUser = useUserStore.getState().user;
    const effectiveUser = currentUser || userStoreUser;
    const currentUserId = effectiveUser?.user?._id || effectiveUser?._id;

    const msgReceiverId = message.receiver?._id || message.receiver;
    if (msgReceiverId === currentUserId) get().markMessagesAsRead();
    //update conversation preivew and unread Count
    set((state) => {
  ...
  markMessagesAsRead: async () => {
    ...
    // Fallback to useUserStore if currentUser is null
    const userStoreUser = useUserStore.getState().user;
    const effectiveUser = currentUser || userStoreUser;
    const currentUserId = effectiveUser?.user?._id || effectiveUser?._id;
    ...
    }).map((mssg) => mssg._id).filter(Boolean);
  ...
  deleteMessage: async (messageId) => {
    try {
      // Don't attempt to delete temporary messages from the server
  ...
  summarizeChat: async (conversationId) => {
    if (!conversationId) return;

    set({ isSummarizing: true, summary: null, error: null });

    try {
      const resp = await axiosClient.get(`/chat/summarize/${conversationId}`);
      if (resp.data.success) {
        set({ summary: resp.data.data.summary, isSummarizing: false });
        return true;
      }
      return false;
    } catch (error) {
      ...
      set({
        error: error.response?.data?.error || "Failed to summarize chat history",
        isSummarizing: false
      });
      return false;
    }
  },
  resetSummary: () => set({ summary: null, isSummarizing: false })
}));
